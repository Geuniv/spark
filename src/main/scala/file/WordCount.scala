...
object WordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 분석할 파일
    val txt = spark.read.textFile("src/resources/comedies") // 읽은 데이터

    import  spark.implicits._

    val words = txt.flatMap(value => value.split("\\s+")) // 파일을 공백 기준 나누기
    val groupedWords = words.groupByKey(_.toLowerCase) // 단어를 소문자로 변환하고, 그룹핑하기
    ...
    println("Total words : " + result.count()) // 결과 단어수 풀력
